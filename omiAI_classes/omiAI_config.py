...
class AIConfig:
    def __init__(self, file):
        ...
        self.ownerID = self.cfg.get('discordBotOwnerID', None)
        ...
        if self.fixSystemPrompt:
            logger.critical("Note: systemPromptFix is enabled, it should be only used if A: You are using external API; B: You get errors from the provider.")

        if not any(model.get('id') == self.currModel for model in self.cfg.get("models")): 
            logger.critical(f'Model {self.currModel} not in model list! You probably will encounter issues if the model in defaultModelID is not valid!')
    ...
    def initConfig(self):
        ...
        try:
            ...
            raise Exception(f"Config failed to load : {e}")